...
public class Result {
  ...
  public String toString() {
    if (!this.finished) return "[?|" + this.query + "]";
    else {
      if (this.proof.isKnown()) return "[T|" + this.query + "]";
      else return "[F|" + this.query + "]";
    }
  }
  ...
  public Proof getProof() throws NoSuchElementException {
    if (this.isFinished() && this.proof != null) return this.proof;
  ...
  public void provenBy(Proof proof) throws IllegalArgumentException {
    if (this.needsEval()) {
      this.proof = proof;
      this.finished = true;
    } else throw new IllegalArgumentException("assertion is already proven");
  }
