...
public class Unification {
  ...
  public String toString() {
    if (!this.success) return "_|_";

    Object[] deltas = this.theta.values().toArray();
    if (deltas.length == 0) return "{}";

    String str = "{";
    for (int i = 0; i < deltas.length; i++) str += ((i == 0) ? " " : ", ") + deltas[i];
    str += " }";
    return str;
  }
  ...
  public void compose(Unification other) {
    if (other.hasFailed() || this.hasFailed()) this.fails();
      ...
      for (Substitution s : this.theta.values()) {
        ...
        for (Substitution t : other.theta.values()) t.substitute(s);

        // Add any missing keys from this
        if (!other.theta.containsValue(s)) other.theta.put(s.from, s);
      }
