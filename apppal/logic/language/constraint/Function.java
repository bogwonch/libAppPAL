...
public class Function extends CE implements Unifiable<CE> {
  ...
  public Function(String name, List<CE> args) {
    this.name = name;
    if (args == null) this.args = new LinkedList<>();
    else this.args = args;
  }
  ...
  public String toString() {
    ...
    if (!this.args.isEmpty()) {
      ...
      for (int i = 1; i < this.args.size(); i++) str += ", " + this.args.get(i);
    }
    return str + ")";
  }
  ...
  public Set<Variable> vars() {
    ...
    for (final CE e : this.args) vars.addAll(e.vars());
    return vars;
  }

  public Set<Constant> consts() {
    final Set<Constant> consts = new HashSet<>();
    for (final CE e : this.args) consts.addAll(e.consts());
    return consts;
  }
  ...
  public Function substitute(Map<Variable, Substitution> delta) {
    ...
    for (final CE arg : this.args) args.add(arg.substitute(delta));
