...
public class Predicate extends VP implements EntityHolding {
  ...
  public Predicate(String name, List<E> args) {
    this.name = name;
    if (args == null) this.args = new LinkedList<E>();
    else this.args = args;
  }
  ...
  public String toString() {
    ...
    if (!this.hasArgs()) return this.name;

    // Else this.args.size() >= 1
    String answer = this.name + "(";
    for (int i = 0; i < this.args.size() - 1; i++) answer += this.args.get(i) + ", ";
  ...
  public Set<Variable> vars() {
    ...
    if (this.hasArgs()) for (E arg : this.args) vars.addAll(arg.vars());
    return vars;
  }
  ...
  public Set<Constant> consts() {
    ...
    if (this.hasArgs()) for (E arg : this.args) consts.addAll(arg.consts());
    return consts;
  }

  public Unification unify(VP vp) {
    final Unification unification = new Unification();
    if (!(vp instanceof Predicate)) return new Unification(false);

    final Predicate other = (Predicate) vp;

    if (!this.name.equals(other.name)) return new Unification(false);

    final int n = this.args.size();
    if (n != other.args.size()) return new Unification(false);
  ...
  public Predicate substitute(Map<Variable, Substitution> delta) {
    ...
    for (E e : this.args) args.add(e.substitute(delta));
