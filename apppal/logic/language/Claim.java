...
/** AppPAL claims fact [ if fact,* ] [ where c ] */
public class Claim implements EntityHolding, Unifiable {
  ...
  public Claim(Fact f, List<Fact> a, Constraint c) {
    ...
    if (a == null) this.antecedents = new LinkedList<>();
    else this.antecedents = a;

    if (c == null) this.constraint = new Sat();
  ...
  public String toString() {
    ...
    if (!this.constraint.isTriviallyTrue()) answer += " where " + this.constraint;
  ...
  public Set<Variable> antecedentVars() {
    ...
    if (this.hasAntecedents()) for (Fact f : this.antecedents) vars.addAll(f.vars());
    return vars;
  }
  ...
  public Set<Constant> antecedentConsts() {
    ...
    if (this.hasAntecedents()) for (Fact f : this.antecedents) consts.addAll(f.consts());
    return consts;
  }
  ...
  public Claim substitute(Map delta) {
    ...
    for (final Fact a : this.antecedents) antecedent.add(a.substitute(delta));
