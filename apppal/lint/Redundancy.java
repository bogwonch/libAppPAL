...
public class Redundancy {
  ...
  private class Proof implements Comparable<Proof> {
    ...
    public Proof search_replace(final Goal target, Set<Goal> replacements) {
      ...
      for (final Goal g : this.goals) {
        if (g.equals(target)) result.goals.addAll(replacements);
        else result.goals.add(g);
      }
      return result;
    }
  ...
  private class Graph {
    ...
    public void dump(OutputStream o) {
      ...
      for (final Goal key : this.goals) {
        ...
        if (links != null && links.is_flat()) goals.append(" color=green");
        else if (links != null && links.is_reduced(this)) goals.append(" color=blue");
        else if (links != null && links.is_simple(this)) goals.append(" color=red");
        goals.append("];\n");
        if (links != null) rules.append(links.graph_dump(key));
      }
    ...
    public void add(final Goal head, final Node tail) {
      ...
      if (node == null) this.graph.put(head, tail);
      else node.merge(tail);
    ...
    public void fix_backrefs() {
      for (final Node n : this.graph.values()) n.fix_backrefs(this);
    }

    // Runs one iteration of the graph flattening algorithm
    public int flatten() {
      List<Node> to_flatten = new LinkedList<>();
      for (final Node n : this.graph.values()) if (n.needs_flattening(this)) to_flatten.add(n);
      ...
      for (final Node n : to_flatten) n.flatten(this);
      this.fix_backrefs();
        ...
        for (final Proof a : node.proofs)
          for (final Proof b : node.proofs)
            if (a == b) continue;
              ...
              else Util.warn(node.owner + " has redundant proofs");
            }
    ...
    public class Node implements Comparable<Node> {
      ...
      public class BackRef implements Comparable<BackRef> {
        ...
        public BackRef(final Node node) {
          this.node = node;
        }
      ...
      public Node(final Goal owner, final Set<Proof> proofs) {
        this(owner);
        for (final Proof proof : proofs) this.add(proof);
      }
        ...
        for (final Proof proof : proofs)
          for (final Goal goal : this.goals()) {
            ...
            if (update != null) {
              ...
              if (should_add) {
                update.used_by.add(r);
              }
            } else Util.warn("policy may be incomplete: " + goal);
          }
      }
      ...
      public Set<Goal> goals() {
        ...
        for (final Proof p : this.proofs) result.addAll(p.goals);
        return result;
      }
      ...
      public int compareTo(Node o) {
        ...
        } else return this.owner.compareTo(o.owner);
      }
    }
  }
  ...
  public Redundancy(final AC ac) {
    ...
    for (final Assertion a : this.ac.assertions) graph.add(a);
    ...
    try {
      ...
      do {
        ...
        if (flattened > 0) graph.dump(new FileOutputStream("graph-" + (iterations++) + ".dot"));
      } while (flattened > 0);
