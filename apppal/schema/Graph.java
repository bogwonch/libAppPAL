...
public class Graph {
  ...
  public class Entity implements Comparable<Entity> {
    ...
    public int compareTo(final Entity other) {
      if (this.is_specific && !other.is_specific) return -1;
      if (!this.is_specific && other.is_specific) return +1;
      else return this.representation.compareTo(other.representation);
    }
  }
  ...
  public class SchemaGraph {
    ...
    private class Path {
      ...
      public Path(final String from, final String to) {
        this(from, to, "");
      }
    ...
    private String add_context(final Entity e) {
      return add(e.toString(), contexts);
    }

    private String add_speaker(final Entity e) {
      return add(e.toString(), speakers);
    }

    private String add_predicate(final String e) {
      return add(e, predicates);
    }
    ...
    public String toString() {
      ...
      for (final Map.Entry<String, String> e : contexts.entrySet())
        out.append(
            "  " + e.getValue() + " [shape=ellipse color=red label=\"" + e.getKey() + "\"]\n");
      out.append("\n");
      for (final Map.Entry<String, String> e : speakers.entrySet())
        out.append(
            "  "
                + e.getValue()
                + " [shape=parallelogram color=blue label=\""
                + e.getKey()
                + "\"]\n");
      out.append("\n");
