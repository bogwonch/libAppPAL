...
public class Lint {
  ...
  public Lint(
    ...
    if (policies.size() == 0) Util.warn("no files to check");
    ...
    if (this.check_completeness) {
      ...
      if (problems.isEmpty()) Util.info("no completeness problems");
      else {
        ...
        for (final Assertion a : check.undecidable()) System.out.println("  " + a);
        System.out.println("");
  ...
  public static void main(final String[] args) {
    ...
    for (final String arg : args) {
      switch (arg) {
        ...
        default:
          if (arg.startsWith("-")) Util.warn("unrecognised flag: '" + arg + "': ignoring");
          else policies.add(arg);
          break;
      }
    }
